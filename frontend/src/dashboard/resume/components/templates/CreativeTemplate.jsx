...
function CreativeTemplate({ resumeInfo }) {
    ...
    const renderDescription = (text) => {
        if (!text) return null;
        if (text.includes('<li>')) {
            return <div className="prose-creative mt-2" dangerouslySetInnerHTML={{ __html: text }} />;
        }
        ...
        return <p className="text-[11px] text-gray-600 mt-2">{text}</p>;
    };
            ...
            <div className="relative">
                ...
                <div className="relative px-8 pt-8 pb-6">
                    <div className="flex items-start gap-6">
                        {/* Avatar */}
                        <div
                            className="w-24 h-24 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg flex-shrink-0"
                            style={gradientStyle}
                        >
                            {resumeInfo?.photoUrl ? (
                                <img
                                    src={resumeInfo.photoUrl}
                                    alt=""
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2 mt-3">
                                {resumeInfo?.email && (
                                    <span className="inline-flex items-center gap-1.5 text-[10px] px-3 py-1 rounded-full bg-gray-100 text-gray-600">
                                ...
                                {resumeInfo.Experience.map((exp, index) => (
                                    <div key={index} className="relative">
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span
                                                className="text-[9px] px-2 py-1 rounded-full text-white font-medium shadow-sm"
                                                style={gradientStyle}
                                            >
                                                {exp?.startDate} - {exp?.currentlyWorking ? 'Present' : exp?.endDate}
                                            </span>
                                        </div>
                                        {renderDescription(exp?.workSummery)}
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {resumeInfo.skills.map((skill, index) => (
                                    <span
                                        key={index}
                                        className="text-[10px] px-2.5 py-1.5 rounded-lg font-medium text-white shadow-sm"
                                        style={gradientStyle}
                                    >
                                        {skill?.name}
                                    </span>
